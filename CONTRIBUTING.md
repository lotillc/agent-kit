# Contributing to @lotiai/agent-kit
...
The package targets Node 24+ and uses pnpm (see `packageManager` in `package.json`).

## Pull requests

- Open against `main`. CI runs `pnpm compile` and `pnpm test` on Node 24.
- One logical change per PR. Keep diffs reviewable.
- Add or update tests for any behavioral change.
- Tests are hermetic: no real process spawn, no real git, no real disk I/O past `node_modules`.
- Avoid introducing new dependencies unless necessary; prefer the standard library or existing deps.

## Reporting issues

File issues at https://github.com/lotillc/agent-kit/issues. Include a minimal reproduction where possible.
